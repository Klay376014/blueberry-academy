...
這個檔案定義 blueberry-academy 的領域詞彙。這個領域的詞有很多陷阱 —— 「一場對戰」在 Bo3
底下是有歧義的，「隊伍」有兩層意義，「同一支隊」的定義取決於賽制。任何人（或 agent）在
寫程式碼、命名變數、討論需求前，先讀這裡。
...
**Bring（選出）**
VGC 每場實際帶上場的 4 隻（log 的 `|teamsize|p1|4`）。
