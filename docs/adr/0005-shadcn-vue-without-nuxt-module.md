...
不安裝 `shadcn-nuxt`。元件放在 `app/components/ui/<name>/`，靠 Nuxt 預設的
path-prefix 自動匯入取得 `Ui` 前綴。
...
**`nuxt.config.ts` 的 `components: [{ path: '~/components', extensions: ['vue'] }]`
是這個決定的必要配套，不能拿掉。** shadcn 在每個元件目錄放一支 `index.ts` barrel
（匯出 `buttonVariants` 與 cva 型別）。Nuxt 預設連 `.ts` 一起掃描成元件，於是
`index.ts` 與 `Button.vue` 都宣告自己叫 `UiButton`：

```
[NUXT_B3011] Two component files resolving to the same name `UiButton`:
 - apps/web/app/components/ui/button/index.ts
 - apps/web/app/components/ui/button/Button.vue
...
`init` 若在第 2 點中止，`app/lib/utils.ts`（元件 import 的 `cn` helper）不會被建出來，
需手動補。
