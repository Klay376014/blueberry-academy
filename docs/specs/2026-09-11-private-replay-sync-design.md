...
只能在要密碼的地方誠實寫明，技術上無解。**UI 必須講清楚密碼會被送到哪裡、留存多久
（不留存）、以及可以改密碼一鍵失效所有 session（§2.2.8）。**

這四點警語的位置與措辭在 #179 之後改過一次，見 **D5**。承諾沒有變，變的是它出現的
時機與用字。
...
**D5 — 密碼問在對話框裡，不常駐在頁面上；警語不說「session」。**
#179 的第一版把私人同步做成匯入頁上的第三段表單：自己的名稱欄、密碼欄與四點警語
一路攤在頁面上，與上面「同步帳號」那段各問一次 Showdown 名稱。改成**一個名稱欄配兩顆
按鈕**（公開直接跑、私人才開對話框問密碼），理由有三，由重到輕：

1. **兩個名稱欄能安靜地對不起來。** 兩欄都預填第一個綁定名稱，使用者改了一邊，另一邊
   還是舊的 —— 而且沒有任何畫面會說出這件事。公開與私人 replay 本來就屬於同一個帳號，
   問兩次是在製造一個不該存在的狀態。
2. **密碼只在需要時存在。** 頁面靜止時 DOM 裡沒有密碼欄，離開對話框的每一條路
   （取消、Esc、關窗、同步完成）都把它清成空字串。這是 §5.1「架構上就沒有東西可洩漏」
   往前端再走一步，不是新的承諾，是同一個承諾少一個例外。
3. **名字先驗再開窗。** PS 只准帳號本人列自己的私人 replay（§2.2.6），所以沒綁定的
   名稱擋在開窗之前 —— 讓人打完密碼才被拒絕，是白打一次。

代價是私人同步多一次點擊。願意付：它換掉的是一個常駐在畫面上的密碼欄。

措辭上把 **session 全部換掉**。§2.2 用這個字是對的（那是 PS 原始碼裡的東西），但警語
的讀者是玩家，不是讀這份文件的人 —— 一段讀不懂的保證等於沒有保證。改成「一張進
Showdown 的通行證，清單讀完就作廢」，英文用 _pass_。講的仍然是 sid，一個字都沒有放寬。
測試把「不出現 session 這個字」當成一條斷言釘住。
