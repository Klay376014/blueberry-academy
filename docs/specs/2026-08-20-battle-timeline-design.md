...
**清單第四次擴充：改寫能力變化的那一族（#123）。** 前三次補的都是「說不出」——
少一枚 chip、少一行事件。這次補的是唯一一個會**說錯**的地方：狀態列的能力變化只由
`-boost`/`-unboost` 累加而成，於是任何**改寫**它的行都不存在，黑霧（`|-clearallboost`）
之後那枚 +2 攻擊會一直留在畫面上。少一枚 chip 是資訊不足，留一枚假的則會讓
「這回合值不值得換」的判斷整個反過來。

收下七行，各自的語意不同，狀態層必須分開處理：

| 行                                               | 招式                         | 語意                                           |
| ------------------------------------------------ | ---------------------------- | ---------------------------------------------- |
| `\|-clearallboost`                               | 黑霧                         | 全場歸零。實測就是裸的一行，不指名任何一隻     |
| `\|-clearboost\|POKEMON`                         | 清除之煙                     | 一隻歸零                                       |
| `\|-setboost\|POKEMON\|STAT\|N`                  | 腹鼓                         | 設成某個值，不是加減                           |
| `\|-swapboost\|SOURCE\|TARGET\|STATS`            | 心之互換／防守互換／力量互換 | 兩隻交換指定的幾項；心之互換整欄省略，交換全部 |
| `\|-copyboost\|SOURCE\|TARGET`                   | 精神強化                     | SOURCE 複製 TARGET 的                          |
| `\|-invertboost\|POKEMON`                        | 上下顛倒                     | 正負反轉                                       |
| `\|-clearpositiveboost\|TARGET\|POKEMON\|EFFECT` | 暗影偷盜                     | 只清掉正向的，負向留著                         |

**`-setboost` 的數字要夾在 ±6。** 這行不一定給的是階數：憤怒穴位（Anger Point）報的是
`12`，Showdown 自己的 client 把它讀成「最大化」而不是一個數字。原封照收的話狀態列會出現
`atk +12` —— 一個對戰不可能處在的狀態。夾在遊戲真有的六階上，畫面說的才是實際發生的事。

**`-clearnegativeboost`（白色草藥）刻意不收。** 它一律帶 `[silent]`，在既有的 silent
規則下已經被丟掉，Showdown 自己也不畫 —— 那是規則生效，不是漏讀。原始碼在這一族的
case 上方留了一行說明，避免下一個人把它當成缺口補上去。唯一不帶 silent 的形狀是第七世代
Z 招式的 `[zeffect]`，只有 gen 7 replay 走得到，這張票沒有讀它（code review 提出）。

**實測比次數更重要的一件事：現有 fixture 證明不出這個 bug。** repo 內 10 份 fixture
（1631 個事件）只有 `-clearallboost` 出現，7 次，全在 `gen9ou-2667293085` 這一場 OU。
unknown 從 356 降到 349，正好是這 7 行 —— 其餘六種 0 次。但關鍵在於：把這 7 次逐一還原
場上狀態，**每一次場上都沒有任何一隻帶著能力變化**。最清楚的是第 23 回合 Corviknight
用鐵壁拿到 `def +2`，第 24 回合牠先被換成 Blissey，黑霧才出現 —— 既有的「離場清空」規則
早就把那枚 chip 收走了，於是 snapshot 剛好正確。

也就是說，**fixture 全綠不代表這件事是對的**。這一族的測試一律用合成 log：狀態層的
測試先讓兩邊都帶上能力變化，再送黑霧進去，才看得見原本留在畫面上的那枚假 chip。

擴充同時要付的兩筆帳：`rowMessage` 為每個新事件補上 en 與 zh-TW 的句子（少了句子就會
多出一行只有圖示沒有字的列，#120 踩過），以及 `-swapboost` 帶進來的新識別字 ——
一串以逗號相連的能力名，交給 `statListDisplayName` 逐一查表後以 `·` 相連，在
`name-coverage.spec.ts` 併入既有的「能力名」那一列（同 `fieldAbilities` 的先例：
fixture 今天照不到，欄位先備著）。`-clearpositiveboost` 第三欄的 effect 不上畫面 ——
那個招式自己的行就在上一列，重複說一次不會讓讀者多知道什麼。

**第五次不是擴充清單，而是把一個已讀的行讀完整：`-ability` 的第三欄（#205）。** 前四次
補的都是整個不存在的型別，這次補的是一個已經在清單上的行**沒讀完**。實測 12 份 fixture
的 35 行 `-ability`：22 行第三欄是 `boost`（威嚇、加速），13 行什麼都沒有（壓迫、緊張感）
—— Showdown 自己用這一欄表示「這則宣告是為了接下來的能力變化」。解析器原本只讀特性名，
把它丟了。

留著它的理由與 T26 同一條：能力的升降今天完全不上主線（`MAIN_LINE` 沒有 `boost`，
`resultOf` 也不收），而要把威嚇的兩個 `-unboost` 畫在威嚇那一行上，配對依據只有兩種
—— 這一欄，或「往前找最近一行」。後者正是 T5／T26 拒絕過的東西。**這一欄只說
「這則宣告關於一個能力變化」，不說「後面哪幾行是它」**；後者是顯示層的讀法，由 #207
連同它的條件一起訂。

照 log 原樣留成字串而不是讀成布林旗標，與 T7 同一條：不認得的標記留在事件上而不是消失。
kwarg 不算標記 —— 被複製的特性把 `[from] ability: Trace` 放在威嚇放 `boost` 的同一格，
與 `-swapboost` 省略能力欄時的形狀相同，兩處共用同一個判斷。

**第六次同樣是把已讀的行讀完整：能力變化那一族的 `[from]`（#206）。** 招式造成的能力
變化要畫在招式那一行上，而配對條件必須只讀 log 自己填過的欄位，否則就是 T5／T26 拒絕過
的「往前找最近一個 `|move|`」。已有的欄位只夠回答「這一行是關於誰」，答不出「這一行是
誰造成的」—— 弱點保險在挨打的那一隻身上發動，那一隻正是該招指名的目標，光看對象會把
道具的作為算到招式頭上。

`[from]` 就是 log 自己填的那一格，於是這一族六個事件（`-boost`/`-unboost`、`-clearboost`、
`-setboost`、`-invertboost`、`-swapboost`、`-copyboost`）一律照 `sourceOf` 收下它。
`-clearallboost` 不收：它誰都沒指名，折給誰的問題根本不會發生。
...
| T27 | UI 的能力變化配對   | 來源沒說或說的就是該招 ∧ 該招指名了它 | 只看對象；`from === null` 一刀切；一律不折    |
| T28 | UI 的特性配對       | `boost` 標記開場域，任何別的行關場域  | 只看「緊接在後面」；往回找最近的 `-ability`   |
...
**T27 是 T26 的同一條規則，換一族行再問一次（#206）**，兩處的差別只在 T26 的第一個條件
寫成 `from === null`，而能力變化這一族寫成「來源沒說，或說的就是這一招」。理由是這一族的
`[from]` 真的會指回招式自己：腹鼓寫 `[from] move: Belly Drum`，心之互換寫
`[from] move: Heart Swap`。T26 的論述本來就是「log 自己說了來源，**而那個來源不是這一招**」
—— 傷害的行從來沒有指回自己的，所以當時 `from === null` 就夠；能力變化有，所以得把那句話
照字面寫出來。憤怒穴位的 `[from] ability: Anger Point` 是反例的一半（它掛在挨了會心一擊
的那一隻身上，而那隻正是該招的目標），弱點保險是另一半。

比對時**連命名空間一起比**，不先把 `move:` 剝掉：那一半正是「說話的是哪一類東西」的
唯一憑據，而發條（Metronome）同時是招式名與道具名 —— 只比裸名的話，道具的作為會折進
同名的招式那一行。

折不進去的一律自成一行並且**上主線**：沒有招式開著的（回合末結算）、log 指給別人的
（道具、特性）、以及黑霧那種誰都沒指名的。它們決定了後面的回合，而上面沒有一行可以讀成
「那是它做的」—— 與 `fieldEffect` 上主線的理由同一條。

實測 12 份 fixture 的 80 行 `-boost`/`-unboost`：48 行折進招式，32 行留在主線上，而**那
32 行恰好就是前面有 `-ability` 宣告的那 32 行**。也就是說 T27 畫出來的界線與 #207 要處理
的那一族剛好互補，沒有一行被招式的作用域吃掉 —— 那是 #207 的驗收條件之一，這裡先量到。

**T28 是同一個問題的第三次，這次沒有 `[from]` 可讀（#207）。** 威嚇的 `-unboost` 是裸的
一行：它不說誰造成的，也不說它屬於上面哪一行。能讀的只有 #205 收下的第三欄標記 ——
Showdown 自己用 `boost` 表示「這則宣告是為了接下來的能力變化」。

標記開場域，**任何不是能力變化的行關場域**。標記說的是「這則宣告關於一個能力變化」，
沒說要往下讀幾行，所以總得有東西說；「直到別的事發生為止」是能成立的最短的一句。實測
19 則帶標記的宣告裡有 1 則（`gen9ou-2667299955` 第 18 回合）後面一行都收不到 —— 威嚇宣告
了而降階沒發生。標記是對宣告本身的陳述，不是對後面幾行的承諾，那一則就留成光禿禿的一行。

帶 `[from]` 的不收：宣告那一行沒有任何名字可以讓 `[from]` 對上，所以指名了來源的那一行
指的一定不是這一列。實測 32 行全是裸的，這正是放行的那個形狀。

**被降能力的那隻畫在點之後，不是箭頭之後。** `-ability` 那行沒有說它瞄準了誰，箭頭會
宣稱一個 log 沒說的方向 —— 與 `effect` 對 `[of]` 的處理（#152）同一條。招式那條路徑走
箭頭，因為目標欄是 log 自己列的。落在特性持有者自己身上的（加速在回合末對自己發動）
畫在該行本身：行首已經有那隻的圖示，點後面再畫一次等於同一件事說兩遍。

**場域的先後**：特性的場域問在招式的前面。威嚇降的那一隻常常正是開著的那一招指名的目標，
而且那一行是裸的 —— T27 要的條件全都成立。兩者都能主張同一行時，宣告是比較近的那一個，
而且它是 log 自己寫下的。反方向不必特別處理：`|move|` 那一行本身就會關掉宣告的場域。

**`-ability` 也關掉招式的場域**，與 `|move|`、`|switch|` 並列進 `CLOSES_ACTION`。只把宣告
問在前面還不夠：宣告收不下的那些（中間夾了別的行）會掉回 T27 的條件，於是又被招式接走 ——
票上說這種要「照 #206 的第 3 條，自成一行上主線」。實測整個 fixture 只有 5 行會落在
「招式還開著，而且前面出現過 `-ability`」這個位置，而那 5 行全都是宣告帶進來的能力變化：
沒有任何傷害或命中結果靠這條路折進招式行，所以這是對現況的 no-op。
