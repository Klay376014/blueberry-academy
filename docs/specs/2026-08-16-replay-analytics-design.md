...
寶可夢 / 招式 / 道具**的 id**（`toID()` 後的形式：signature、DB 欄位、對照表的 key）是識別碼不是文案，**永不翻譯**。

> **這一行已被 [ADR-0014](../adr/0014-localised-species-names.md) 部分推翻（#101）。**
> 原本寫的是「寶可夢 / 招式 / 道具名稱永遠只顯示英文，不進 i18n」。畫面上**給人看的
> 名字**現在會本地化：寶可夢名已經做了（走產生出來的對照表，不走語系檔），招式與道具
> 留給 #102 / #103。哪些用語本地化、哪些留作識別碼，以 ADR-0014 的表為準。
...
**隊伍表現**：一列一支隊，可展開下鑽到 bring 組合並標示筆數。「含不完整場次」這個全域開關
**預設開啟** —— 先讓使用者看到打過的全部，需要時自己收窄，而不是一開始就默默少掉幾場；
關掉之後，bring 組合與最近場次列表都會濾掉 `bring_complete = false` 的對戰，
但**總勝率、趨勢圖與隊伍層級的勝率一場都不濾**（註冊的六隻不管那場怎麼結束都是已知的，
勝負也是真的勝負，所以兩層本來就該有不同的分母）。場次列表以 series 為單位判斷：
整串每場都不完整才整串消失，只要有一場完整就整串留著 —— 從中間挖掉一場會讓列表標的
戰次和抽屜對不起來。排序用 **Wilson score 下界**，
