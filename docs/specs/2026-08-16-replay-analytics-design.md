...
**賽制（`format_id`）是必填篩選器**（#16 實作時修訂，見 Q30）。跨規則的勝率回答不了
任何人的問題，而兩個賽制的天梯 rating 是兩個不同的數，畫成一條線就是錯的。頁面載入時
自動選場次最多的那個賽制。既然選了賽制就等於決定了是不是 Bo3，原本的 Bo1/Bo3 切換
按鈕隨之移除。
...
| Q30     | 賽制篩選     | 必填，預設場次最多者                                                                 | 可選「全部賽制」（跨規則混算無意義）                    |
